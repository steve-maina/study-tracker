  ...
  const triggerBtn = (
    <FormButtons
      ...
      disabled={
        ...
        isPending ||
        (props.operation === "create" ? props.timeElapsed < 60 : false)
      }
      onClick={clearDialogInputs}
    >
      Save Session
    </FormButtons>
  );
      ...
      <Form action={formAction} ref={formRef}>
        {props.operation === "create" ? (
          <p
            className={clsx(
              "text-center",
              !props.isTimerRunning && props.timeElapsed > 0 && "blinking"
            )}
          >
            {formatTimeElapsed(props.timeElapsed.toString())}
          </p>
        ) : (
          <p className="text-center">Save a past study session</p>
        )}
        ...
        <input
          type="hidden"
          value={recordingEnd.toISOString()}
          name="recordingEnd"
        />
        <div
          className={clsx(
            "flex mt-4",
            props.operation === "create" && "justify-between",
            props.operation == "save" && "justify-center"
          )}
        >
          {props.operation === "create" ? (
            <>
              <FormButtons
                type="button"
                disabled={isPending}
                onClick={props.isTimerRunning ? pauseTimer : startTimer}
              >
                {props.isTimerRunning
                  ? "Pause"
                  : props.timeElapsed > 0
