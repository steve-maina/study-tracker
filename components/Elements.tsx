      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 bg-white">
          ...
          <div>
            ...
            <input required type="text" className="bg-gray-400 ml-2" value={props.topic} onChange={(event:React.ChangeEvent<HTMLInputElement>)=>{
              if(event.target.value){
              props.setTopic(event.target.value )}}}/></label>
           {props.operation === "save" ?  <><label className="block mx-2 mt-4"> Date:
              <input required type="date" className="bg-gray-400 ml-2 mb-4" value={props.date} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                if (event.target.value) {
                  props.setDate(event.target.value);
                }
              } } /></label><label className="">Start Time
                <input required type="time" value={props.startTime} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                  props.setStartTime(event.target.value);
                } } className="mx-4 p-4" />
              </label><label>End Time
                <input required type="time" value={props.endTime} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
